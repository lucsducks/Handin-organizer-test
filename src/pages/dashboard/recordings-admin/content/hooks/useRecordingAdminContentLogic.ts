...
export const useRecordingAdminContentLogic = () => {
  const { recordingId } = useParams<{ recordingId: string }>();
  const { selectedRecording, getAdminRecording } = useRecordingStore();
  ...
  useEffect(() => {
    if (recordingId) {
      getAdminRecording(Number(recordingId));
    }
  }, [recordingId, getAdminRecording]);

  const handleResourceClick = async (resource: Resource) => {
    if (
      resource.type === ResourceTypes.VIDEO &&
      resource.file &&
      selectedRecording
    ) {
      try {
